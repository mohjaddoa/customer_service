...
def handle_booking():
    ...
    while True:
        ...
        if status == "break":
            ...
            You assist Dr. John, who is available for appointments on weekdays (Monday to Friday) from 9:00 AM to 5:00 PM.
...
st.title("Customer Service - Appointment Booking using AI Agent(Sarah)")

# Add a subtitle
st.subheader("Book your appointment with Dr.John using your voice !")

# Inform the user about the functionality
st.write("Choose an option below:")
st.write("1- click on preparation to prepare voice system")
st.write("2- click on start button to start conversation,and follow instructions")
st.write("Note: Any time you want stop conversation say STOP or Exit")

if st.button("Prepartion"):
    st.write(extractor.download_spacy_model())


# Create two buttons for booking and canceling appointments
if st.button("start"):
    # st.write("start button")
    ...
    names = list(names)
    for name in names:
        if name not in ["John","john","dr","Dr","Sarah","sarah"]:
            #   print(name)
            customer_name=name
    st.write("names :",customer_name)
    st.write("date and time :",unique_values)


# Footer message
st.write("This is a prototype,and you might encounter some unrealistic response or inaccurate results")
st.write("Developed by Dr.Mohammed Jaddoa")
